...
class configuration
{
	public:
		...
		void ConfigInit(const std::string& delimiter="\t");
...
{
	size_t len_sur = this -> sur.size();
	size_t len_var_st = this -> var_st.size();
	...
	for (size_t i = 0; i < len_sur; ++i)
	{
		...
		sur_tr.push_back( std::thread(  [this, i, &delimiter/*, &fut_sur[i]*/] 
		{
			std::stringstream ss = leo::ReadFile(this -> sur[i].file_name);
			survey sur = SurParser(ss, delimiter);
			sur.file_name = this -> sur[i].file_name;
			this -> sur[i] = sur;
			//fut_var[i].set_value();
		}
		));
	...
	{
		var_st_tr.push_back( std::thread( [this, i, &delimiter]
		{
			std::stringstream ss = leo::ReadFile(this -> var_st[i].file_name);
			var_station var = VarParser(ss, delimiter);
			var.file_name = this -> var_st[i].file_name;
			var.X = this -> var_st[i].X;
			var.Y = this -> var_st[i].Y;
			this -> var_st[i] = var;
		}
		));
	}
...
void configuration::dT_varInit()
{
	size_t len = this -> sur.size();

	std::vector<std::thread> sur_tr;
	
	for (size_t i = 0; i < len; ++i)
	{
		sur_tr.push_back( std::thread(  [this, i]
		{
			int j = 0;
			
			std::vector<var_station> vr_st;

			for (auto& it : this -> var_st)
			{
				if ( covers(it.Date_start, it.Time_start, it.Date_stop, it.Time_stop,
						this -> sur[i].Date_start, this -> sur[i].Time_start,
						this -> sur[i].Date_stop, this -> sur[i].Time_stop) )
			...
			if (j >= 3)
			{
				for (size_t k = 0; k < this -> sur[i].meas.size(); ++k)
				{
					double dT_v = dT_var(this -> sur[i], vr_st[0], vr_st[1], vr_st[2], k);
					this -> sur[i].meas[k].dT_var = dT_v;

					this -> sur[i].meas[k].dT_bot = this -> sur[i].meas[k].T_bot - dT_v;

					if (this -> sur[i].T_grad_init_)
					{
						this -> sur[i].meas[k].dT_top = this -> sur[i].meas[k].T_top - dT_v;
					}
				}

				this -> sur[i].dT_var_init_ = true;
			}
			else if (j == 2)
			{
				for (size_t k = 0; k < this -> sur[i].meas.size(); ++k)
				{
					double dT_v = dT_var(this -> sur[i], vr_st[0], vr_st[1], k);
					this -> sur[i].meas[k].dT_var = dT_v;
			...
			else if (j == 1)
			{
				for (size_t k = 0; k < this -> sur[i].meas.size(); ++k)
				{
					double dT_v = dT_var(this -> sur[i], vr_st[0], k);
			...
			else
			{
				std::cerr << "dT_varInit: Error of searching station in range time! file:"
						<< this -> sur[i].file_name << "\n";
				std::cerr << "Excepted Date range: " << this -> sur[i].Date_start[0] << "."
								<< this -> sur[i].Date_start[1] << "."
								<< this -> sur[i].Date_start[2] << " - "
								<< this -> sur[i].Date_stop[0] << "." 
								<< this -> sur[i].Date_stop[1] << "."
								<< this -> sur[i].Date_stop[2] << "\n"; 
...
void configuration::T_anom_varInit()
{
	size_t len = this -> sur.size();
	...
	for (size_t i = 0; i < len; ++i)
	{
		sur_tr.push_back( std::thread(  [this, i]
		{
			...
			if (j >= 3)
			{
				for (size_t k = 0; k < this -> sur[i].meas.size(); ++k)
				{
					std::vector<double> T_a = T_anom_var(this -> sur[i], vr_st[0], vr_st[1], vr_st[2], k);

					this -> sur[i].meas[k].T_bot_anom = T_a[0];
					if (this -> sur[i].T_grad_init_) this -> sur[i].meas[k].T_top_anom = T_a[1];
				}

				this -> sur[i].T_anom_init_ = true;
			} 
			else if (j == 2) 
			{
				for (size_t k = 0; k < this -> sur[i].meas.size(); ++k)
				{
					std::vector<double> T_a = T_anom_var(this -> sur[i], vr_st[0], vr_st[1], k);
			...
			else if (j == 1)
			{
				for (size_t k = 0; k < this -> sur[i].meas.size(); ++k)
				{
					std::vector<double> T_a = T_anom_var(this -> sur[i], vr_st[0], k);
...
void configuration::WriteConfig(bool create_file, bool to_file, bool print_var, bool Unit_meas, std::string out_file)
{
	if (create_file)
	{
		if (to_file)
...
int InputParser(int argc, char* argv[], bool& vars, bool& to_file, bool& create_file, bool& print_var, bool& T_an_var, bool& Unit_meas, bool& Level, std::string& delimiter, std::string& config_file, std::string& out_file)
{
	if (argc < 4)
	{
...
inline int pipline(int argc, char* argv[])
{
	...
	config.ConfigInit(delimiter);

	std::vector<std::thread> processing;

	if (vars) processing.push_back( std::thread(&configuration::dT_varInit, std::ref(config)) );
	if (T_an_var) processing.push_back( std::thread(&configuration::T_anom_varInit, std::ref(config)) );
	...
	if (Level) config.Leveling();


	config.WriteConfig(create_file, to_file, print_var, Unit_meas, out_file);
